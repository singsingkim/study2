...
for learning_rate in learning_rates : 
    model.compile(loss='mse', optimizer=Adam(learning_rate=learning_rate), metrics=['mse', 'mae', 'acc'])
    start_time = tm.time()
    hist = model.fit(x_train, y_train, epochs=100, batch_size=70, validation_split=0.3, verbose=0) #98
    end_time = tm.time()
